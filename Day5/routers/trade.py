...
async def get_status(
    ...
    result = await db.execute(select(models.Portfolio).where(models.Portfolio.username == user.id))
...
async def trade(
    ...
    if action == "buy":
        ...
        if p:
            p.avg_price = (
                (p.avg_price * p.amount) + (payload.amount * payload.price)
            ) / (p.amount + payload.amount)
            p.amount += payload.amount
        # 2. 기존 데이터가 없는 경우: 새로운 models.Portfolio 객체를 생성(new_p)하고 db.add() 하세요.
        else:
            new_p = models.Portfolio(username=user.id, amount=payload.amount, avg_price=payload.price, symbol="oz")
            db.add(new_p)

    elif action == "sell":
        # TODO: 매도 가능 여부를 체크하세요. (포트폴리오가 없거나, 보유 수량 < 매도 요청 수량일 경우 400 에러)
        if p is None or p.amount < payload.amount:
